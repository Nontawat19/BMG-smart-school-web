...
const main = async () => {
  ...
  const target = students.filter(d => {
    const f = val({mapValue:{fields:d.fields}});
    return (f.studentId === '08235') || (String(f.firstName||'').includes('ณัฐชา') && String(f.lastName||'').includes('ชุทอง'));
  });
  console.log(`Found ${target.length} matching students`);
  target.forEach(d => {
    const id = d.name.split('/').pop();
    const f = val({mapValue:{fields:d.fields}});
    console.log(`id=${id} studentId=${f.studentId} name=${f.title||''}${f.firstName} ${f.lastName} class=${f.classLevel||f.classId}`);
  });
};
