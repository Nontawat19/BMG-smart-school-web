...
const main = async () => {
  ...
  console.log(`Total ClassroomAttendance records for this student: ${records.length}`);
  const bySubject = {};
  records.forEach(d => {
  ...
  Object.entries(bySubject).forEach(([code, recs]) => {
    console.log(`${code}: ${recs.length} records, statuses: ${recs.map(r=>r.status).join(',')}`);
    console.log(`  sample dates/rooms: ${recs.slice(0,3).map(r=>`${r.date?.slice(0,10)} room=${r.room} grp=${r.groupNumber}`).join(' | ')}`);
  });
};
