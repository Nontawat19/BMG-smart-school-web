...
import { FaSchool, FaUserTie, FaUserGraduate, FaMapMarkerAlt, FaMoneyBillWave, FaUsers, FaTasks, FaLayerGroup } from 'react-icons/fa';
...
const SkeletonLoader: React.FC = () => (
  <div className="max-w-4xl mx-auto animate-pulse">
    ...
    <div className="bg-white dark:bg-[#2a2b2f] rounded-2xl p-6 shadow-sm dark:shadow-none">
      <div className="h-8 w-1/3 bg-gray-300 dark:bg-gray-700 rounded mb-6 pb-4 border-b border-transparent"></div>
...
const ViewSchoolInfoPage: React.FC = () => {
  ...
  const fetchData = useCallback(async () => {
    if (!schoolId) {
      setIsLoading(false);
      Swal.fire('เกิดข้อผิดพลาด', 'ไม่พบรหัสโรงเรียนใน URL', 'error');
    ...
    try {
      ...
      if (docSnap.exists()) {
        setInfo(docSnap.data() as SchoolInfo);
      } else {
        setInfo(null); // No data found
        Swal.fire('ไม่พบข้อมูล', 'ไม่พบข้อมูลโรงเรียนที่ต้องการ', 'warning');
        navigate('/owner/schools');
      }
    } catch (error) {
            ...
            {info ? (
              <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
                <div className="md:col-span-4 flex flex-col items-center">
                  ...
                  <h2 className="text-2xl font-bold text-center" title={info.schoolName}>{info.schoolName || 'ยังไม่มีชื่อโรงเรียน'}</h2>
                  <div className="flex flex-col items-center">
                    {info.schoolCode && <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">รหัสสถานศึกษา: {info.schoolCode}</p>}
                    {info.schoolAbbreviation && <p className="text-gray-500 dark:text-gray-400 text-center">({info.schoolAbbreviation})</p>}
                  </div>
                  <p className="text-gray-500 dark:text-gray-400 text-center mt-1">{info.affiliation || 'ยังไม่มีสังกัด'}</p>
                </div>
...
export default ViewSchoolInfoPage;