                        ...
                        <div className={`w-full min-w-0 flex flex-col gap-1.5 ${isPwaMode ? 'mt-0' : 'mt-4 sm:mt-2'}`}>
                            ...
                            <div className="grid grid-cols-4 gap-1.5">
                                {statusOptions.filter(opt => opt.status !== 'escape').map(opt => (
                                    <button
                                        ...
                                        className={`
                                            flex min-w-0 items-center justify-center py-2 rounded-xl font-bold transition-all duration-200 ${isPwaMode ? 'text-[11px]' : 'text-xs'}
                            ...
                            {statusOptions.filter(opt => opt.status === 'escape').map(opt => (
                                <button
                                    ...
                                    disabled={isSubmitted || isHoliday || studentLeaves[student.id]}
                                    className={`
                                        w-full flex min-w-0 items-center justify-center py-1.5 rounded-xl font-bold transition-all duration-200 gap-1.5 ${isPwaMode ? 'text-[11px]' : 'text-xs'}
                                        ${attendance[student.id] === opt.status
