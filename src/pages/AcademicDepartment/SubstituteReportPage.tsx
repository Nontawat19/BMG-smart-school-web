...
import { Calendar, ChevronLeft, ChevronRight, FileText, Printer, RefreshCw, Search, X, FileDown } from "lucide-react";
import {
...
const SubstituteReportPage: React.FC = () => {
  ...
  const buildSubstituteReportDocument = () => (
    <SubstituteReportPdfDocument data={buildPdfData()} />
  );
  ...
  const handleGeneratePdf = async () => {
    ...
    try {
      const blob = await pdf(buildSubstituteReportDocument()).toBlob();
          ...
          >
            <div className="flex items-center justify-between border-b border-gray-200 px-5 py-4 dark:border-white/10">
              <h2 className="text-base font-bold text-gray-900 dark:text-white">
                ตัวอย่างเอกสาร — รายงานการสอนแทน
              </h2>
