...
export const TeacherScheduleHeader: React.FC<TeacherScheduleHeaderProps> = ({
    ...
    return (
        <header className="relative z-40 bg-white/80 dark:bg-[#2a2b2f]/80 backdrop-blur-2xl border-b border-gray-100 dark:border-white/5 shadow-sm px-3 lg:px-4 py-1.5 transition-all">
            <div className="w-full max-w-[1600px] mx-auto flex flex-wrap items-center gap-3">
                <div className="shrink-0 flex items-center">
                    <div className="shrink-0 flex items-center gap-2.5">
                        <BackButton to="/academic/hub/scheduling" className="shrink-0 w-9 h-9" />
                        <div className="shrink-0 flex items-center gap-2.5 group cursor-pointer">
                            ...
                            <div className="shrink-0 flex flex-col">
                                <h1 className="whitespace-nowrap text-base lg:text-lg font-black tracking-tight text-gray-900 dark:text-white uppercase leading-tight">ระบบจัดตารางสอนอัจฉริยะ</h1>
                                <div className="mt-0.5 flex flex-nowrap items-center gap-1.5">
                                    <span className="hidden min-[1500px]:inline shrink-0 whitespace-nowrap text-[9px] font-black text-gray-700 dark:text-gray-400 uppercase tracking-[0.2em]">Academic Management Console</span>
                                    ...
                                    <div className="shrink-0 flex items-center gap-1.5">
                                        ...
                                        <span className="whitespace-nowrap text-[9px] font-black text-emerald-500 uppercase tracking-[0.16em] leading-none">System Operational</span>
                                    </div>
                                </div>
                            </div>
                        </div>
                ...
                <div className="shrink-0 flex items-center gap-2">
                    <label className="group relative flex h-10 w-[172px] shrink-0 items-center gap-2 rounded-2xl border border-gray-100 bg-gray-50/80 px-3 shadow-sm backdrop-blur-xl transition-colors hover:border-indigo-400/40 dark:border-white/5 dark:bg-white/[0.03]">
                    ...
                    <label className="group relative flex h-10 w-[150px] shrink-0 items-center gap-2 rounded-2xl border border-gray-100 bg-gray-50/80 px-3 shadow-sm backdrop-blur-xl transition-colors hover:border-indigo-400/40 dark:border-white/5 dark:bg-white/[0.03]">
                ...
                <div className="flex flex-wrap items-center gap-2 ml-auto justify-end min-w-0">
                    <button
                        onClick={handleGenerateSchoolTimetable}
                        disabled={isAutoScheduling}
                        className="flex items-center justify-center gap-2 px-3.5 py-2 rounded-2xl bg-indigo-600 hover:bg-indigo-500 transition-all duration-300 text-white shadow-[0_8px_20px_-5px_rgba(79,70,229,0.45)] hover:shadow-[0_12px_28px_-5px_rgba(79,70,229,0.55)] text-[11px] font-black uppercase tracking-normal group h-10 border border-white/20 active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed whitespace-nowrap"
                    >
                        {isAutoScheduling ? <Loader2 size={17} className="shrink-0 animate-spin" /> : <Cpu size={17} className="shrink-0" />}
                        <span className="relative z-10 drop-shadow-[0_2px_4px_rgba(0,0,0,0.3)]">จัดตารางสอนทั้งโรงเรียน</span>
                    </button>
                    <button
                        onClick={handleClearAllSchedules}
                        className="flex items-center justify-center gap-2 px-3 py-2 rounded-2xl border border-rose-600/20 bg-rose-600/5 text-rose-500 hover:bg-rose-600/10 hover:border-rose-500/35 transition-all duration-300 text-[11px] font-black uppercase tracking-normal group h-10 active:scale-95 whitespace-nowrap"
                    >
                        <X size={17} className="shrink-0" />
                        <span className="relative z-10">ลบทั้งหมด</span>
                    </button>
                    <button
                        onClick={handleSaveSchedule}
                        disabled={isSaving}
                        className="flex items-center justify-center gap-2 px-3.5 py-2 rounded-2xl bg-gradient-to-r from-indigo-600 via-violet-600 to-indigo-600 bg-[length:200%_auto] hover:bg-right transition-all duration-500 text-white shadow-[0_8px_20px_-5px_rgba(79,70,229,0.5)] hover:shadow-[0_12px_28px_-5px_rgba(79,70,229,0.6)] text-[11px] font-black uppercase tracking-normal group h-10 border border-white/20 active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed whitespace-nowrap"
                    >
