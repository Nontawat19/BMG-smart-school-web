...
const systemConfigDetails: Record<string, { name: string, description: string }> = {
    school: { name: 'Telegram Bot หลักของโรงเรียน', description: 'บอทประจำโรงเรียนสำหรับติดต่อสอบถามและประชาสัมพันธ์ภาพรวม' },
...
const TelegramManagementPage: React.FC = () => {
    ...
    const handleCloseModal = () => {
        ...
        setFormData({ telegramBotToken: "", telegramChatId: "", enableNotification: false });
    };
    ...
    const handleSave = async (e: React.FormEvent) => {
        ...
        try {
            if (selectedTarget.type === 'teacher') {
                ...
                const schoolDocRef = doc(firestore, "school-settings", selectedSchoolId);
                const updatePath = `telegramSettings.${selectedTarget.key}`;
            ...
            Swal.fire({
                icon: 'success',
                title: 'บันทึกสำเร็จ',
                text: 'ข้อมูล Telegram API ถูกบันทึกเรียบร้อยแล้ว',
                timer: 1500,
            ...
            console.error("Error saving Telegram API settings:", error);
                    ...
                    <div className="bg-white dark:bg-[#2a2b2f] p-4 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 mb-6">
                        <div className="w-full md:w-1/3">
                            ...
                            <select
                                ...
                                className="w-full px-3 py-2 rounded-lg border border-gray-200 dark:border-gray-600 bg-gray-50 dark:bg-[#1e1f21] text-gray-900 dark:text-white outline-none focus:ring-2 focus:ring-blue-500 disabled:opacity-60 disabled:cursor-not-allowed"
                            >
                                {schools.map(school => (
                    ...
                    <div className="bg-white dark:bg-[#2a2b2f] p-4 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 mb-6">
                        <div className="relative max-w-md">
                            ...
                            <input
                                ...
                                className="w-full pl-10 pr-4 py-2 rounded-lg border border-gray-200 dark:border-gray-600 bg-gray-50 dark:bg-[#1e1f21] text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500 outline-none"
                            />
                        </div>
                    </div>
                                ...
                                <div key={grade} className="space-y-4">
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                                        {groupedTeachers[grade].map((teacher) => {
                                            const isConfigured = !!teacher.telegramBotToken && !!teacher.telegramChatId;
                                            return (
                                                <div key={teacher.id} className="bg-white dark:bg-[#2a2b2f] rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6 flex flex-col hover:shadow-md transition-shadow">
                                                    ...
                                                    <div className="mt-auto pt-4 border-t border-gray-100 dark:border-gray-700">
                                                        <div className="flex justify-between items-center mb-3">
                                                            <span className="text-xs text-gray-500 dark:text-gray-400">สถานะ Telegram</span>
                ...
                {isModalOpen && selectedTarget && (
                    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
                        <div className="bg-white dark:bg-[#2a2b2f] rounded-2xl shadow-xl w-full max-w-lg overflow-hidden animate-fadeIn">
                            <div className="px-6 py-4 border-b border-gray-200 dark:border-gray-700 flex justify-between items-center bg-blue-50 dark:bg-blue-900/10">
                                <h3 className="text-lg font-bold text-gray-900 dark:text-white flex items-center gap-2">
                                    <Send className="w-5 h-5 text-blue-600" />
                                    ตั้งค่า Telegram {selectedTarget.type === 'teacher' ? `(${selectedTarget.data.homeroomGrade})` : `(${selectedTarget.name})`}
                                </h3>
                            ...
                            <form onSubmit={handleSave} className="p-6 space-y-4">
                                <div className="bg-blue-50 dark:bg-blue-900/20 p-3 rounded-lg text-sm text-blue-800 dark:text-blue-300 mb-4">
                                    <p className="mb-2">กรุณาระบุข้อมูล Telegram Bot Token จาก @BotFather และ Chat ID</p>
                                    <a
                                    ...
                                    >
                                        <ExternalLink className="w-3 h-3" /> ไปที่ BotFather ใน Telegram
                                    </a>
                                </div>
                                ...
                                <div className="flex items-center gap-3 p-4 bg-gray-50 dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700">
                                    ...
                                    <div className="flex-1">
                                        ...
                                        <p className="text-xs text-gray-500 dark:text-gray-400">
                                            เมื่อเปิดใช้งาน ระบบจะส่งข้อความแจ้งเตือนไปยัง Telegram นี้
                                        </p>
                                    </div>
                                    <input
                                        type="checkbox"
                                ...
                                <div>
                                    <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                                        Bot Token (จาก @BotFather) <span className="text-red-500">*</span>
                                    </label>
                                    <input
                                        type="text"
                                        ...
                                        className="w-full px-3 py-2 rounded-lg border border-gray-200 dark:border-gray-600 bg-gray-50 dark:bg-[#1e1f21] text-gray-900 dark:text-white outline-none focus:ring-2 focus:ring-blue-500 font-mono text-xs"
                                    />
                                </div>
                                ...
                                <div>
                                    ...
                                    <input
                                        ...
                                        className="w-full px-3 py-2 rounded-lg border border-gray-200 dark:border-gray-600 bg-gray-50 dark:bg-[#1e1f21] text-gray-900 dark:text-white outline-none focus:ring-2 focus:ring-blue-500"
                                    />
                                </div>
