...
const styles = StyleSheet.create({
  ...
  subHeaderMiddle: {
    height: 60, // พื้นที่สำหรับ "คะแนนก่อนกลางภาค"
    flexDirection: 'row',
    borderBottomWidth: 1,
    borderColor: '#000',
  },
  ...
  verticalTextContainer: {
    width: 100, // ความกว้างสมมติของข้อความที่หมุน
    height: 20,
    justifyContent: 'center',
    alignItems: 'center',
...
const ExamSummaryPage: React.FC<ExamSummaryPageProps> = ({
  academicYear,
  selectedClass,
  termToDisplay,
  FULL_CLASSES,
  ...
  const VerticalHeader = ({ text, width, max, style = {} }: { text: string, width: number, max?: number, style?: any }) => (
    <View style={[styles.subHeaderGroup, { width, ...style }]}>
      <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center', borderBottomWidth: max !== undefined ? 1 : 0, borderColor: '#000' }}>
      ...
      {max !== undefined && (
        <View style={{ height: 20, justifyContent: 'center', alignItems: 'center' }}>
          <Text style={styles.cellText}>{max}</Text>
        </View>
      )}
    </View>
  );
    ...
    <PdfPage orientation="portrait">
      ...
      <View style={styles.table}>
        ...
        <View style={styles.headerContainer}>
          <VerticalHeader text="เลขที่" width={COL_WIDTHS.NO} />
          <VerticalHeader text="เลขประจำตัว" width={COL_WIDTHS.ID} />

          <View style={[styles.subHeaderGroup, { width: COL_WIDTHS.NAME, justifyContent: 'center', alignItems: 'center' }]}>
            <Text style={styles.cellText}>ชื่อ - สกุล</Text>
          </View>

          {/* Formative Scores Header Group */}
          <View style={{ width: formativeSectionWidth, flexDirection: 'column', borderRightWidth: 1, borderColor: '#000' }}>
            {/* ชั้นที่ 1: ชื่อวิชา */}
            ...
            <View style={{ flex: 1, flexDirection: 'row' }}>
              ...
              <View style={[styles.innerGroup, { width: preWidth }]}>
                ...
                <View style={styles.innerGroupSlots}>
                  {preSlots.map((_, i) => (
                    <View key={`pre-h-${i}`} style={[styles.cell, { width: COL_WIDTHS.SCORE_ITEM, borderRightWidth: i === 4 ? 0 : 1, borderBottomWidth: 0 }, !preMidtermAssessments[i] ? styles.bgGray : {}]}>
              ...
              <View style={[styles.innerGroup, { width: postWidth }]}>
                ...
                <View style={styles.innerGroupSlots}>
                  {postSlots.map((_, i) => (
                    <View key={`post-h-${i}`} style={[styles.cell, { width: COL_WIDTHS.SCORE_ITEM, borderRightWidth: i === 4 ? 0 : 1, borderBottomWidth: 0 }, !postMidtermAssessments[i] ? styles.bgGray : {}]}>
              ...
              <View style={{ width: COL_WIDTHS.SCORE_SUM, flexDirection: 'column' }}>
        ...
        {studentChunk.map((s) => {
          ...
          return (
            <View key={s.id} style={styles.row}>
              ...
              {/* 6. รวมเก็บ (พื้นหลังเทาเฉพาะช่องรวม) */}
